...
from app.services import audio_separation, transcription, synthesis, media_downloader, linguistics
...
def convert_to_wav(input_path: str) -> str:
    ...
    try:
        ...
        stream = ffmpeg.output(stream, output_path, acodec='pcm_s16le', ar='44100', ac=2)
...
def process_lyrics(self, prev_result: dict):
    ...
    try:
        ...
        update_job_progress(job_id, "PROCESSING", 50)
        return prev_result
...
def process_linguistics(self, prev_result: dict):
    ...
    try:
        ...
        if use_mock:
            ...
            lyrics_segments = linguistics.translate_and_romanize(lyrics_segments, target_lang="ko")
        ...
        if "lyrics" in prev_result and "segments" in prev_result["lyrics"]:
             ...
             prev_result["lyrics"] = linguistics._add_mock_translation(prev_result["lyrics"])

        update_job_progress(job_id, "PROCESSING", 75)
        return prev_result
...
def upload_to_storage(file_path: str, job_id: str) -> str:
    ...
    try:
        ...
        s3.upload_file(
            ...
            s3_key,
            ExtraArgs={'ContentType': 'video/mp4', 'ACL': 'public-read'} # ACL might fail on some buckets
        )
        ...
        if settings.AWS_ENDPOINT_URL:
             ...
             public_url = f"{settings.AWS_ENDPOINT_URL}/{settings.AWS_BUCKET_NAME}/{s3_key}"
        else:
...
def render_video(self, prev_result: dict):
    ...
    try:
        ...
        # Finalize
        update_job_progress(job_id, "COMPLETED", 100, result={"output_path": final_url})
